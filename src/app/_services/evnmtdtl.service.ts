...
export class EvnmtdtlService {
  ...
  getAll(EvnmtId: number): Observable<IEvnmtdtl[]> {
    console.log(this.endpointUrl + "evnmtdtl?evnmt_id=" + EvnmtId);
    return this.http.get(this.endpointUrl + "evnmtdtl?evnmt_id=" + EvnmtId)
  ...
  getEvnmtdtl(id: number): Observable<IEvnmt> {
        return this.http.get(this.endpointUrl + "evnmtdtl/" + id).map((response: Response) => {
    ...
    return this.http.post(this.endpointUrl + "evnmtdtl", JSON.stringify(evnmtdtl),
